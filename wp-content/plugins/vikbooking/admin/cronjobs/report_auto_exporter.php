    ...
    if (vbo_cron_report_auto_exporter_profiles_list.hasOwnProperty(report)) {
        ...
        if (profiles_list.length) {
            example_payload = Object.assign(example_payload, {
                _reportProfile: profiles_list.join(', ')
            });
        }
    }
...
HTML;
    ...
    protected function execute()
    {
        ...
        // check if a specific profile setting ID was defined
        if (($report_payload['_reportProfile'] ?? '') && is_string($report_payload['_reportProfile']) && $report_obj->allowsProfileSettings()) {
            // sanitize the report profile instruction (only one profile supported, but all suggested separated with a comma)
            $profile_parts = explode(',', $report_payload['_reportProfile']);
            $report_payload['_reportProfile'] = trim($profile_parts[0]);
            // load all report profiles
            ...
            $report_obj->setActiveProfile($report_payload['_reportProfile']);
        }
        ...
        if (!$data_exported) {
            ...
            if ($def_export_method) {
                ...
                if ($custom_export_error) {
                    $this->output('<p>Errors with the report execution: ' . $custom_export_error . '</p>');
                    $this->appendLog('Errors with the report execution: ' . $custom_export_error);
                } else {
                    $this->output('<p>Report execution completed with an error.</p>');
                    $this->appendLog('Report execution completed with an error.');
                }
            }
        ...
        if ($def_export_method) {
            ...
            // set output and log
            $this->output('<p>Report executed successfully.</p>');
            $this->appendLog('Report executed successfully.');
        }
        ...
        if ($resources && $email_addr) {
            ...
            if ($vbo_app->sendMail($sender, $sender, $email_addr, $sender, $subject, rtrim($content, "\n"), $is_html = false, 'base64', $resource_attachs)) {
                $this->output('<p>Resource file successfully sent via email to: ' . $email_addr . '</p>');
                $this->appendLog('Resource file successfully sent via email to: ' . $email_addr);
            }
        }
