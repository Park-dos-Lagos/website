...
JS
	...
	protected function registerCheckins($scope = null, array $data = [])
	{
		...
		try {
			...
			$was_valid = $response->code == 200 && !strcasecmp((string) $xmlResponse->ValidationResultDTO->IsValid, 'true');
	...
	protected function registerEndDay($scope = null, array $data = [])
	{
		...
		$day_dt = new DateTime(date('Y-m-d 00:00:00', $day_ts));
		$day_dt->setTimezone(new DateTimeZone('UTC'));
		$day_utc = $in_dt->format('Y-m-d\TH:i:s.v\Z');
