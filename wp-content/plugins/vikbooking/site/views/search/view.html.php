								...
								if (is_array($arrtar) && $arrtar) {
									...
									if (count($results) == $proomsnum) {
										...
										if (!empty($proomdetail) && array_key_exists($proomdetail, $arrtar) && $proomsnum == 1) {
											...
											$mainframe->redirect(JRoute::_('index.php?' . http_build_query($url_query_args), false));
											exit;
										} else {
											if (!empty($proomdetail) && $proomsnum == 1) {
												...
												if ($room_missing === false) {
													...
													$session->set('vboSearchRoomId', $proomdetail);
													//
													$mainframe->redirect(JRoute::_("index.php?option=com_vikbooking&task=showprc&roomsnum=".$proomsnum.$roomopt_str.$aduchild_str."&days=".$daysdiff."&checkin=".$first."&checkout=".$second.(!empty($ppkg_id) ? "&pkg_id=" . $ppkg_id : "").(!empty($pitemid) ? "&Itemid=" . $pitemid : ""), false));
													// VBO 1.11 - push data to tracker and close
